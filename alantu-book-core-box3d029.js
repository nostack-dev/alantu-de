import { createAlantuBookPhysics } from "./alantu-book-physics-box3d026.js";
...
export function createAlantuBookCore(options){
  ...
  function pointerMove(e){
    ...
    if(dragState.mode===null){
      ...
      if(closedSide==="start"&&dx<0){
        ...
        dragState.grabU=dx<0
          ? clamp((dragState.startX-visualHingeX)/visualPageWidth,.08,.98)
          : clamp((visualHingeX-dragState.startX)/visualPageWidth,.08,.98);

        activeTurn={index,progress:dragState.startProgress,velocity:0,target:dragState.startProgress};
        physics.ensureBuilt(currentLeaf);
        physics.beginGrab(index,dragState.grabU,dragState.startProgress);
      }else{
        return;
      }
    }
  ...
  function releaseDrag(cancelled=false){
    ...
    if(dragState.mode==="leaf")physics.endGrab();
