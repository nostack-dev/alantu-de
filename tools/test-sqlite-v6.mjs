...
import {openMarketStore,persistObservation,persistPrediction,persistOutcome,persistEvidence,persistExperiment,storeStats,V6_CONTRACT_ID,V6_CONTRACT_TEXT} from '../market-relay/sqlite-store.mjs';
...
persistObservation(s,{s:'ORCL',t,p:123,dv:10,day_volume:100,recv_at:t+900}); // duplicate transport event
...
assert.equal(rows[0].received_at_ms,t+900); // metadata refresh did not make a sample

const p={id:'v6-1-'+t,version:'yahoo-monetary-dt-v6',horizon_minutes:1,entry_market_ms:t,entry_market_at:new Date(t).toISOString(),
...
persistOutcome(s,{...p,status:'evaluated',endpoint_market_at:new Date(t+60000).toISOString(),endpoint_received_at:new Date(t+60250).toISOString(),
 ...
 structural_profitable:true,learned_gross_bps:5,learned_net_bps:3,learned_profitable:true});
...
persistEvidence(s,{evidence_key:'source:test',kind:'source_event',approach_version:'source-event-v1',symbol:'ORCL',event_at_ms:t,payload:{headline:'x'}});
persistExperiment(s,{approach_version:'yahoo-monetary-dt-v6',status:'collecting',config:{clock:'market_event_time'}});

const st=storeStats(s);
assert.deepEqual({events:st.events,predictions:st.predictions,outcomes:st.outcomes,evidence:st.evidence,experiments:st.experiments},{events:2,predictions:1,outcomes:1,evidence:1,experiments:1});
